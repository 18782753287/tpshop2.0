...
class GoodsLogic extends Model
{
    ...
    public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        $list_spec = array();
        $old_spec = $filter_param['spec'];//1_5@2_6   5_   @6_
    ...
    {
        $goods_id_str = implode(',', $goods_id_arr);$goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        ...
        $old_attr = $filter_param['attr'];//1_fsd@2_fd
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        $psize = ceil($max_price / $c); // 每一段累积的价钱 111 5 23
        $parr = array();
        for ($i = 0; $i < $c; $i++) {
            $start = $i * $psize;//0-23 23-46 46*69 69*92 92*115
            $end = $start + $psize;
